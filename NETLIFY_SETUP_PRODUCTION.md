# 🔧 Настройка фронтенда на Netlify для работы с бэкендом на Synology

## ✅ Статус деплоя бэкенда

Бэкенд должен быть запущен на Synology. Проверьте статус:

```bash
# На Synology
cd /volume1/shortsai/app/backend
pm2 status
# или
/usr/local/bin/pm2 status
```

Если бэкенд не запущен, запустите вручную:
```bash
cd /volume1/shortsai/app/backend
pm2 start dist/index.js --name shortsai-backend
pm2 save
```

## 🌐 Переменные окружения для Netlify

### 1. Backend URL (ОБЯЗАТЕЛЬНО)

Ваш бэкенд доступен через VPS по адресу:

```
VITE_BACKEND_URL=http://185.104.248.130:5001
```

**Важно:** Используйте именно этот URL, так как он проброшен через VPN туннель на Synology.

### 2. Firebase Configuration (ОБЯЗАТЕЛЬНО)

Эти переменные нужны для работы с Firebase Authentication и Firestore:

```
VITE_FIREBASE_API_KEY=AIzaSy...your-api-key-here
VITE_FIREBASE_AUTH_DOMAIN=prompt-6a4fd.firebaseapp.com
VITE_FIREBASE_PROJECT_ID=prompt-6a4fd
VITE_FIREBASE_STORAGE_BUCKET=prompt-6a4fd.firebasestorage.app
VITE_FIREBASE_MESSAGING_SENDER_ID=905027425668
VITE_FIREBASE_APP_ID=1:905027425668:web:38f58912370df2c2be39d1
```

**Где получить:**
1. Откройте [Firebase Console](https://console.firebase.google.com/)
2. Выберите проект `prompt-6a4fd`
3. Перейдите в **Project Settings** → **Your apps** → **Web app**
4. Скопируйте значения из раздела **SDK setup and configuration**

### 3. OpenAI Model (ОПЦИОНАЛЬНО)

Модель OpenAI для генерации сценариев:

```
VITE_OPENAI_MODEL=gpt-4o-mini
```

## 📝 Инструкция по добавлению в Netlify

1. **Откройте Netlify Dashboard**
   - Перейдите на https://app.netlify.com/
   - Выберите ваш сайт

2. **Перейдите в настройки переменных окружения**
   - **Site settings** → **Build & deploy** → **Environment variables**
   - Или **Site settings** → **Environment variables**

3. **Добавьте переменные**
   - Нажмите **Add variable** для каждой переменной
   - Введите **Key** (имя переменной) и **Value** (значение)
   - Выберите **Scopes**:
     - ✅ **Production** - для продакшена
     - ✅ **Deploy previews** - для preview деплоев
     - ✅ **Branch deploys** - для деплоев из веток (опционально)

4. **Сохраните и пересоберите**
   - Нажмите **Save**
   - Запустите новый деплой: **Deploys** → **Trigger deploy** → **Deploy site**

## ✅ Полный список переменных для копирования

Скопируйте и заполните значения:

```env
# Backend URL (ОБЯЗАТЕЛЬНО) - ваш бэкенд на Synology через VPS
VITE_BACKEND_URL=http://185.104.248.130:5001

# Firebase (ОБЯЗАТЕЛЬНО)
VITE_FIREBASE_API_KEY=AIzaSy...your-api-key-here
VITE_FIREBASE_AUTH_DOMAIN=prompt-6a4fd.firebaseapp.com
VITE_FIREBASE_PROJECT_ID=prompt-6a4fd
VITE_FIREBASE_STORAGE_BUCKET=prompt-6a4fd.firebasestorage.app
VITE_FIREBASE_MESSAGING_SENDER_ID=905027425668
VITE_FIREBASE_APP_ID=1:905027425668:web:38f58912370df2c2be39d1

# OpenAI Model (ОПЦИОНАЛЬНО)
VITE_OPENAI_MODEL=gpt-4o-mini
```

## 🔍 Проверка работоспособности

### 1. Проверьте бэкенд

```bash
# Проверка с вашего компьютера
curl http://185.104.248.130:5001/health

# Должен вернуть: {"ok":true}
```

### 2. Проверьте фронтенд

После деплоя на Netlify:
1. Откройте сайт в браузере
2. Откройте консоль разработчика (F12)
3. Проверьте, что нет ошибок подключения к бэкенду
4. Попробуйте войти в систему

### 3. Проверьте переменные окружения

В консоли браузера выполните:
```javascript
console.log('Backend URL:', import.meta.env.VITE_BACKEND_URL);
console.log('Firebase Project:', import.meta.env.VITE_FIREBASE_PROJECT_ID);
```

## ⚠️ Важные замечания

1. **Префикс `VITE_` обязателен** - только переменные с этим префиксом доступны в клиентском коде
2. **Не используйте `VITE_FIREBASE_APY_KEY`** (с опечаткой) - правильное имя `VITE_FIREBASE_API_KEY`
3. **После изменения переменных** обязательно пересоберите сайт в Netlify
4. **CORS настроен** - бэкенд должен принимать запросы с домена Netlify

## 🐛 Устранение неполадок

### Ошибка: "Failed to fetch" или "Network error"

1. Проверьте, что бэкенд запущен:
   ```bash
   # На Synology
   pm2 status
   curl http://127.0.0.1:8080/health
   ```

2. Проверьте, что порт 5001 проброшен на VPS:
   ```bash
   # На VPS
   sudo iptables -t nat -L -n | grep 5001
   ```

3. Проверьте доступность бэкенда извне:
   ```bash
   # С вашего компьютера
   curl http://185.104.248.130:5001/health
   ```

### Ошибка: "CORS policy"

Убедитесь, что в бэкенде настроен CORS для домена Netlify. Проверьте `backend/src/index.ts`:

```typescript
const FRONTEND_ORIGIN = process.env.FRONTEND_ORIGIN || "https://your-site.netlify.app";
app.use(cors({ origin: FRONTEND_ORIGIN }));
```

### Ошибка: "401 Unauthorized"

Проверьте, что Firebase настройки правильные и пользователь авторизован.





