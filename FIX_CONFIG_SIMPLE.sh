#!/bin/bash
# Простое исправление конфигурации Nginx
...
# Создаем правильную конфигурацию через printf (более надежно чем heredoc)
printf '%s\n' \
'server {' \
'    listen 80;' \
'    listen [::]:80;' \
...
'        add_header Access-Control-Expose-Headers "Content-Length,Content-Range,Accept-Ranges" always;' \
'    }' \
'}' > /etc/nginx/sites-available/api.hotwell.synology.me

# Включаем сайт
ln -sf /etc/nginx/sites-available/api.hotwell.synology.me /etc/nginx/sites-enabled/

# Проверяем
nginx -t && systemctl restart nginx && echo "✅ Успешно!" || echo "❌ Ошибка!"
