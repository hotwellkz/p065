...
export async function requestTelegramCode(phoneNumber: string): Promise<{ phoneCodeHash: string }> {
  ...
  const response = await fetch(`${API_BASE_URL}/api/telegram-integration/request-code`, {
    method: "POST",
    headers: {
...
export async function confirmTelegramCode(code: string, password?: string): Promise<{ status: string; message?: string }> {
  const token = await getAuthToken();
  const response = await fetch(`${API_BASE_URL}/api/telegram-integration/confirm-code`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`
...
export async function disconnectTelegram(): Promise<void> {
  const token = await getAuthToken();
  const response = await fetch(`${API_BASE_URL}/api/telegram-integration/disconnect`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`
...
export async function getTelegramStatus(): Promise<TelegramIntegrationStatus> {
  const token = await getAuthToken();
  const response = await fetch(`${API_BASE_URL}/api/telegram-integration/status`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`
    }
...
export async function resetTelegramIntegration(): Promise<{ success: boolean; message?: string }> {
  const token = await getAuthToken();
  const response = await fetch(`${API_BASE_URL}/api/telegram-integration/reset`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`
