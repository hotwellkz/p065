...
export async function generateDriveFolders(channelId: string): Promise<GenerateDriveFoldersResponse> {
  ...
  const response = await fetch(`${API_BASE_URL}/api/channels/${channelId}/generate-drive-folders`, {
    method: "POST",
    headers: {
...
export async function generateDriveFoldersForWizard(params: {
  ...
  const token = await getAuthToken();
  const response = await fetch(`${API_BASE_URL}/api/channels/wizard/generate-drive-folders`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`
