...
const AIAutoGenerateModal = ({
  ...
  const handleFetchVideoToDrive = async () => {
    ...
    try {
      ...
      console.log("[handleSaveToServer] Starting request", {
        channelId: channel.id,
      ...
      console.log("[handleSaveToServer] Response received", {
      ...
      // Обработка ответа с ошибкой (status: "error")
      if (result.status === "error") {
        const errorCode = (result as any).code || (result as any).error;
        const errorMessage = result.message || (result as any).message;

        console.log("[handleSaveToServer] Error response", {
          errorCode,
          errorMessage,
          fullResult: result
        });

        // Обработка NO_VIDEO_FOUND
        if (errorCode === "NO_VIDEO_FOUND") {
        ...
        // Обработка CHANNEL_NOT_FOUND
        if (errorCode === "CHANNEL_NOT_FOUND") {
          setDriveStatus("error");
          setDriveMessage(
            "Канал не найден. Обновите страницу и попробуйте снова."
          );
          return;
        }
        ...
        setDriveStatus("error");
        setDriveMessage(
          errorMessage || "Ошибка сервера при сохранении видео. Попробуйте позже."
        );
        return;
      }

      // Fallback для неизвестного статуса
      console.warn("[handleSaveToServer] Unknown status", result);
      setDriveStatus("error");
      setDriveMessage(
        result.message || "Ошибка сервера при сохранении видео. Попробуйте позже."
      );
    } catch (err: any) {
      setDriveStatus("error");
      ...
      // Обработка NO_VIDEO_FOUND (видео ещё не готово)
      if (errorCode === "NO_VIDEO_FOUND" || errorMessage?.includes("Видео ещё не готово") || errorMessage?.includes("NO_VIDEO_FOUND")) {
        setDriveMessage(
          "Видео ещё не готово в чате SyntX. Подождите окончания генерации и попробуйте ещё раз."
        );
        return;
      ...
      // Обработка CHANNEL_NOT_FOUND (должна быть явной, не путать с NO_VIDEO_FOUND)
      if (errorCode === "CHANNEL_NOT_FOUND" || (err?.response?.status === 404 && errorMessage?.includes("CHANNEL_NOT_FOUND"))) {
        setDriveMessage(
          "Канал не найден. Обновите страницу и попробуйте снова."
        );
        return;
